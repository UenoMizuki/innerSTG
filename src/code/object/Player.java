...
public class Player extends GameObject {
	...
	public Player(double x, double y, int width, int height, double col_x, double col_y, int hp, int damage, int window_w,
	...
	public void update() {
		...
		if (KeyManager.isPressed(Key.get("Z"))) {
			for(int i=0;i<50;i++) {
			ms.getGameScene().objects.add(new Bullet(x+(KeyManager.isPressed(Key.get("RIGHT"))?-0.00025*i:KeyManager.isPressed(Key.get("LEFT"))?0.00025*i:0),y-0.001*i,10,10,5,5,10,1,window_w,window_h,ms));}
		}
	}
}
